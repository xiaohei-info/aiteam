...
class EmployeeRepo:
    ...
    def create(self, emp: Employee) -> Employee:
        self._cur.execute(
            ...
            (emp.id, emp.enterprise_id, emp.template_id, emp.profile_name,
             ...
             emp.temperature, emp.max_tokens,
    ...
    def get_system_planner(self, enterprise_id: str) -> Optional[Employee]:
        """Return the enterprise system planner employee, or None if not seeded yet."""
        self._cur.execute(
            "SELECT id, enterprise_id, template_id, profile_name, display_name, "
            "role_name, status, created_from, model_provider, model_name, "
            "temperature, max_tokens, "
    ...
    def update_status(self, emp: Employee) -> Employee:
        self._cur.execute(
            ...
            "capabilities_json=%s, avatar_url=%s, description=%s, "
    ...
    def _row_to_entity(row) -> Employee:
        return Employee(
            ...
            max_tokens=int(row[11]) if row[11] is not None else 2048,
            prompt_version=row[12], config_version=row[13],
            avatar_url=row[14], description=row[15],
            archive_reason=row[16],
            last_provisioned_at=str(row[17]) if row[17] else None,
            capabilities_json=str(row[18]) if row[18] else "{}",
            created_at=str(row[19]), updated_at=str(row[20]),
            created_by=row[21] or "", updated_by=row[22] or "",
            deleted_at=str(row[23]) if row[23] else None,
        )
