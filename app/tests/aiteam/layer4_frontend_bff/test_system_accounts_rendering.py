...
def _run_system_accounts(after_init_js: str = "return {};", enterprises_js: str | None = None) -> dict:
    enterprises_source = enterprises_js or """
[
...
const fetchCalls = [];
const enterpriseList = {enterprises_source};
...
global.fetch = async (url, options) => {{
  ...
  if (url === '/api/system-admin/enterprises?role=system_admin' && options && options.method === 'POST') {{
    return {{
      ok: true,
      status: 201,
      statusText: 'Created',
...
(async () => {{
  const nodes = {{
    ...
    '[data-role="enterprise-register-form"]': createEventTarget(''),
    '[data-role="enterprise-register-name"]': createEventTarget(''),
    '[data-role="enterprise-register-slug"]': createEventTarget(''),
    '[data-role="enterprise-register-owner"]': createEventTarget(''),
  }};
  const container = {{
    innerHTML: '',
...
def test_system_accounts_empty_state_renders_register_enterprise_entry() -> None:
    payload = _run_system_accounts(enterprises_js="[]")

    assert "注册企业" in payload["html"]
    assert 'data-role="enterprise-register-form"' in payload["html"]
    assert 'data-role="enterprise-register-name"' in payload["html"]
    assert 'data-role="enterprise-register-slug"' in payload["html"]
    assert 'data-role="enterprise-register-owner"' in payload["html"]
    assert "暂无企业账号" in payload["html"]


def test_system_accounts_register_enterprise_posts_to_system_admin_api() -> None:
    payload = _run_system_accounts(
        enterprises_js="[]",
        after_init_js="""
  const nameInput = container.querySelector('[data-role="enterprise-register-name"]');
  const slugInput = container.querySelector('[data-role="enterprise-register-slug"]');
  const ownerInput = container.querySelector('[data-role="enterprise-register-owner"]');
  const form = container.querySelector('[data-role="enterprise-register-form"]');
  if (nameInput) nameInput.value = 'Taiyi Demo Enterprise';
  if (slugInput) slugInput.value = 'taiyi-demo';
  if (ownerInput) ownerInput.value = 'usr_taiyi_admin';
  if (form && form.listeners && form.listeners.submit) {
    await form.listeners.submit({ preventDefault() {}, currentTarget: form, target: form });
  }
  return {};
""",
    )
    ...
    assert {
        "url": "/api/system-admin/enterprises?role=system_admin",
        "method": "POST",
    } in payload["fetchCalls"]
