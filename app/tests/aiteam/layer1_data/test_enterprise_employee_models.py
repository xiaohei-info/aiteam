...
# ═══════════════════════════════════════════════════════════════════
# T06: Repository persistence (requires PG)
# ═══════════════════════════════════════════════════════════════════
...
def _start_ephemeral_postgres() -> tuple[str, int]:
    if shutil.which("docker") is None:
        pytest.skip("PostgreSQL test backend unavailable: docker not installed and no local PG on 5433")

    container_name = f"aiteam-l1-s02-{uuid.uuid4().hex[:8]}"
    subprocess.run(
        [
            "docker",
            "run",
...
def db_conn():
    """Module-scoped PostgreSQL connection for repository tests."""
    import psycopg2
...
def clean_tables(db_conn):
    """Clean mutable tables before each repository test."""
    cur = db_conn.cursor()
    try:
