...
def _start_ephemeral_postgres() -> tuple:
    ...
    container_name = f"aiteam-l1-s04-{uuid.uuid4().hex[:8]}"
    subprocess.run(
        [
...
def db_conn():
    """Module-scoped PostgreSQL connection for L1-S04 repository tests."""
    import psycopg2
...
def clean_tables(db_conn):
    """Clean L1-S04 tables before each repository test."""
    cur = db_conn.cursor()
    try:
