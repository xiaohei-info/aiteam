...
def test_materialize_helper_called_immediately_before_error_path_clears():
    ...
    src = Path(__file__).parent.parent.parent.joinpath('api', 'streaming.py').read_text(encoding='utf-8')
...
def test_cancel_copy_falls_back_to_aifred_for_blank_bot_name(monkeypatch):
    ...
    assert streaming._cancelled_turn_hint() == (
        'The run was cancelled by the user before AIFred finished. '
        'No provider failure occurred.'
    )
