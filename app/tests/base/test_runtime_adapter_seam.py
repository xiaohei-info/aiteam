...
def test_chat_cancel_route_uses_adapter_only_when_flag_enabled():
    ...
    src = (routes.Path(__file__).parent.parent.parent / "api" / "routes.py").read_text(encoding="utf-8")
...
def test_rfc_distinguishes_goal_routing_from_queue_route_staging():
    routes = importlib.import_module("api.routes")
    rfc = (routes.Path(__file__).parent.parent.parent.parent / "docs" / "design-notes" / "app-rfcs" / "hermes-run-adapter-contract.md").read_text(encoding="utf-8")
