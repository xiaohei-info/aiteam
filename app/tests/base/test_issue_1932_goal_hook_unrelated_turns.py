...
def test_streaming_source_code_gates_on_stream_goal_related():
    ...
    streaming_py = (Path(__file__).resolve().parent.parents[2] / "api" / "streaming.py").read_text()
...
def test_routes_reads_pending_goal_continuation():
    ...
    from pathlib import Path
    routes_py = (Path(__file__).resolve().parent.parents[2] / "api" / "routes.py").read_text()
