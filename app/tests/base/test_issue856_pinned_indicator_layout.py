...
SESSIONS_JS = (Path(__file__).resolve().parent.parent.parent / "static" / "sessions.js").read_text()
STYLE_CSS = (Path(__file__).resolve().parent.parent.parent / "static" / "style.css").read_text()
...
def test_sidebar_uses_local_inflight_state_for_immediate_spinner():
    messages_js = (Path(__file__).resolve().parent.parent.parent / "static" / "messages.js").read_text()
