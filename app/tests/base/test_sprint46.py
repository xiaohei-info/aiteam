...
def test_static_commands_js_registers_compress_alias(cleanup_test_sessions):
    ...
    with open(Path(__file__).resolve().parent.parents[2] / "static" / "commands.js", encoding="utf-8") as f:
        src = f.read()
...
def test_static_session_load_resumes_manual_compression_polling(cleanup_test_sessions):
    ...
    with open(Path(__file__).resolve().parent.parents[2] / "static" / "sessions.js", encoding="utf-8") as f:
        src = f.read()
