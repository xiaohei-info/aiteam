...
# ── AI Team namespace dispatch (added above existing route handling) ──

_AI_TEAM_PREFIXES = {
    "/api/team/":               "team_panel.api_team.router_team",
    "/api/enterprise-admin/":   "team_panel.api_team.router_enterprise_admin",
...
def _is_aiteam_page_path(path: str) -> bool:
...
def _try_aiteam_dispatch(handler, path, method, body=None):
    """If path matches an AI Team namespace, delegate to the router stub.

    Returns (dispatched: bool, status_code: int, response_body, content_type: str|None).
    content_type is None for JSON (default), or a specific Content-Type string for non-JSON responses (e.g. 'text/event-stream').
    """
    for prefix, module_name in _AI_TEAM_PREFIXES.items():
        if path.startswith(prefix):
            try:
                ...
                return True, status, resp_body, None
            except ImportError:
                return True, 501, {"error": "router_unavailable"}, None
    return False, 0, {}, None
...
def handle_get(handler, parsed) -> bool:
    ...
    for prefix in _AI_TEAM_PAGE_PREFIXES:
        ...
        if parsed.path.startswith(static_prefix):
            stripped = parsed._replace(path=parsed.path[len(prefix):])
            return _serve_static(handler, stripped)
