...
(function(){
var _aiteamShellPath = null;
try {
  if (window.aiteam && window.aiteam.shell && typeof window.aiteam.shell.matchPath === 'function') {
    _aiteamShellPath = window.aiteam.shell.matchPath(window.location.pathname);
  }
} catch (_e) {}
if (_aiteamShellPath) {
  try {
    // Keep the layout element in the DOM but mark it as hidden so
    // downstream JS that eagerly references .layout won't throw.
    var _layout = document.querySelector('.layout');
    if (_layout) _layout.style.display = 'none';
  } catch (_e) {}
  try {
    var _titlebar = document.querySelector('.app-titlebar');
    if (_titlebar) _titlebar.style.display = 'none';
  } catch (_e2) {}
  // Defer page-shell init to DOMContentLoaded so stylesheet and DOM are ready.
  var _attemptInit = function () {
    try {
      if (window.aiteam && window.aiteam.shell && typeof window.aiteam.shell.init === 'function') {
        window.aiteam.shell.init(_aiteamShellPath);
      }
    } catch (_e3) {}
  };
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _attemptInit, { once: true });
  } else {
    _attemptInit();
  }
  return;  // ← stops normal Hermes boot
}
})();
