...
| 执行面 | 用户端 Agent（本地） | pull 已授权的 provider/model/pricing 快照；run 时由 Driver 最小注入 tenant 受限 Relay Token，用后不写共享 profile |
...
- `RunSpec.provider_ref` 只引用平台 Provider，不内联任何明文凭据。
