...
>
> **架构形态修订（2026-08-26）**：一个 Manager 服务实例只服务一个企业；Operator 才是多企业平台身份/治理面。现有 `tenant_id` token claim 和表字段暂保留以兼容已部署契约，在 Manager 独立账号库/数据库内代表当前企业数据关联，不表示同一 Manager 进程承载多个企业。
...
>
> **形态变更（2026-08-26）**：不设中心 Edge Gateway；Operator 只持系统账号与负责人 bootstrap 来源；每个 Manager 部署是当前企业负责人/成员身份源，按绑定 tenant 持凭据与签名密钥；Agent 只持本会话 token 与本地验签材料。
