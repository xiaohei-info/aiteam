...
采用"**Operator 平台运营 SaaS + Manager 多租户企业管理 SaaS + Agent 用户本地数据面 + 进程内 Pi SessionHost + 外部能力域**"的总体结构，并配套"系统所有权分库 + Manager 租户隔离底座 + 云侧服务间调用 + Agent 主动访问 Manager + 用户端 Pi event/SSE 流式通道 + 治理摘要逐级上报"的通信与数据底座。
...
| 编号 | 议题 | 裁决 | 详见 |
|---|---|---|---|
| D1 | 架构形态 | **Operator 平台运营 SaaS + Manager 多租户企业管理 SaaS + Agent 用户本地数据面**；用户端 Agent Service 内含 Pi SessionHost。〔再次修订早期"Manager 企业自部署"〕 | 00 §4 |
