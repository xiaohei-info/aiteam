...
| F03 | 目录发布 | Operator 创建/发布/下架专家模板、行业方案或平台技能固定版本 → 通知 Manager 可见范围或由 Manager 拉取目录版本 | Operator 持模板/平台技能分发真相；Manager 可缓存租户可见目录索引，且不直连外部技能市场 |
...
| F06 | 招募专家 | Manager 租户内点击招募 → Manager 向 Operator 拉专家模板详情及其中固定版本技能包 → 幂等安装到本 tenant `skill_catalog` → 写入 employee instance，可绑定部门/成员授权 | Operator 不写 Manager 租户库；Manager 不改模板真相；技能 ID/版本/hash 不一致则拒绝招募 |
