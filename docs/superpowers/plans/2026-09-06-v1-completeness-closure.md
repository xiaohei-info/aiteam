...
> **当前状态说明（2026-09-08）**：本文包含 2026-09-06 执行记录，其中出现的 `e5a29890`、`feat/v1-completion-wave1` 等 branch/HEAD 仅是历史快照，不是当前源码或发布基线。当前以 `feature/manager-session-multitenancy-stage-b` 的父控快照、最新独立审查结果及实际 CI checkout SHA 为准；当前累计改动仍未提交/合并/部署。
...
1. 每个Manager tenant 只有一个由 TenantContext 派生/持久化的企业workspace；外部输入不得选择其它企业/任意workspace，同一进程的多个企业会话仍按 JWT/TenantContext + RLS 隔离。统一新增 `server/manager_service/knowledge_access_policy.py` 为有效策略解析点，由 snapshot、授权投影和每次 Manager MCP `search/get` 共用；不能只在Agent工具列表检查。
...
3. Wave2 需要时由 Operator 维护 `manager_deployment` 注册：deployment ID、Manager内/外地址、enterprise/tenant 目标与服务 principal、公钥集合/fingerprint、status/revision。该控制面信任注册不替代当前 Manager 的用户会话级 tenant 隔离；endpoint 变更/身份重绑须显式受限管理动作，不能从业务body自动学习。
4. 最小scope集合按route映射：Manager→Operator `catalog:read`、`relay:resolve`、`rollup:write`、`enterprise-policy:read`，只能对应 tenant；Operator→Manager `enterprise:provision`、`owner:bootstrap`、`catalog:notify`、`enterprise-policy:write`、`notification:write`。rollup、provider、skill/catalog所有服务路由消费principal，body的tenant/enterprise只校验一致，不选择权限范围。Wave2 注册完成后，Manager 只信对应 target audience 与已登记部署 scope；这不改变当前用户 JWT/request 的单 tenant 语义。
...
6. Operator provision/reset/notify/catalog fanout按enterprise解析gateway，不再用进程唯一manager_url。响应返回非秘密Manager handoff地址/deployment引用；owner bootstrap仍一次性给负责人，Operator只存hash。Manager 对 control-plane body 的 tenant 目标做显式校验；当前相位闸未通过前保持 fail-closed，不新增容器编排。
...
1. 历史记录的已部署基线为`5483218e`（PR56），曾记录未提交`e5a29890`；这些值不可作为当前工作树或 release 版本。当前发布必须由主控通过授权只读预检取得实际 checkout SHA、组件/schema、DB备份可用性、服务状态及**非秘密**enterprise/tenant↔Manager deployment/endpoint/public key fingerprint清单；当前多租户部署不得合并/迁移。
...
3. Wave2发布前主控确认每个真实enterprise 的 Manager service-principal 注册与双方 public trust 根、oper/Manager 服务 key 安全生成持有、TLS 地址 allowlist、公开 handoff URL 和受支持客户端组合。当前 TEST 只对已验证的 seeded tenant 做 smoke；在后续相位闸与信任注册完成前，不打开第二 tenant F01，也不能用相同 URL/合成映射伪造证据。
...
- 实际代码落点：`server/manager_service/{knowledge_access_policy,rag_mcp,snapshot_service,authorized_config_service,employee_bindings_repositories,employee_bindings_services,schemas_employee_bindings,routes_employee_bindings,knowledge_intake_repository,routes_grants,app}.py`、上述migration/SQL、`server/shared/contracts/snapshot.py`；必要 `knowledge_intake_service.py`仅保持索引与policy列分离，不改调度。Web新增 `features/experts/KnowledgePolicyPanel.tsx`（及source-named测试），使用当前 tenant knowledge list 与真实管理路由，嵌入 `EmployeeConfigDrawer.tsx`；`features/knowledge/KnowledgePage.tsx`说明document删除与员工deny不同；无普通SPA跨端访问。更新Manager README/OpenAPI由真实typed router导出，不手写副本。
...
- 历史 #1 绑定源记录（已由 2026-09-07 Stage A 覆盖）：当前不使用 `MANAGER_TENANT_ID` 作为进程绑定；Manager verifier/auth service 从 JWT/TenantContext 解析单一 request tenant，缺失或歧义 fail-closed，不扫描或猜测 tenant_registry。healthz 仍存活，readyz 只校验本端 DB/schema；密码链不依赖 `MANAGER_PUBLIC_ORIGIN`，Passkey origin错误只影响Passkey能力。
...
- 历史执行记录（2026-09-06）：当时 branch `feat/v1-completion-wave1`、HEAD `e5a29890`（基线 `5483218e`）；该记录不代表当前分支。当前累计 Stage A/B 改动仍由父控管理，不 reset/clean，不 stage/commit/push/deploy，按最新审查快照收口。
...
- Contract anchors: exact request tenant is derived from the JWT/TenantContext (legacy `MANAGER_TENANT_ID` is ignored) and unbound Manager maintenance is fail-closed; canonical `skills` is presence-aware (`[]` is authoritative; malformed present arrays reject); legacy scalar/object `files` remain inspectable and migration JSONB calls are CASE-safe; direct memory raw receive is bounded before parsing while Pydantic 422/typed 413 and privacy remain; compatibility bind/unbind is typed 404/422 and one tenant transactionally linearized; audience IDs are UUID + current-tenant subjects before publication, platform skill bytes are validated outside but installed inside the atomic publication, and unknown knowledge submissions remain protected/non-retryable until reconciliation.
- Validation targets: focused Manager/Agent/Web tests plus `scripts/check-openapi.sh`; run affected PostgreSQL cases when `ADMIN_DB_URL` supports them and record explicit skips. TEST release records must state application-writer stop order, dependency start before backup/DDL, migration then new-stack start, and distinguish the actual deployed SHA from any uncommitted snapshot. No production fallback or API weakening.

### Wave1 final independent-review fix pass execution record (2026-09-06 17:49 UTC, worker fix3)

- 历史执行基线（2026-09-06）曾为 branch `feat/v1-completion-wave1`、HEAD `e5a29890`、base `5483218e`；当前以最新 Stage A/B 父控快照为准。本段列出的七个安全/data seam 已并入当前累计改动，不得据此宣称已发布。
...
- 历史 final-review 记录（2026-09-06）曾以 branch `feat/v1-completion-wave1`、HEAD `e5a29890`、base `5483218e` 为基线；当前以最新 Stage A/B 父控快照为准。本段所列 review findings 已并入当前累计改动，未据此宣称发布。
...
- Required failure-first checks remain: each Manager auth operation that can return `principal_inactive`, `password_reset_required`, or `password_expired` declares a 403 example; unbound Manager `/readyz` declares 503; direct memory POST/PATCH rejects over-limit bodies before decoding and exposes 413 plus metadata/body bounds; state/examples validate; knowledge binding examples use bounded config and UUID audiences; scoped Web memory methods require `employee_id`; native list mapping handles `ListMemoryUnitsResponse.type`/`fact_type`; runtime/lease docs and OpenAPI agree on `aiteam-memory-v1`, allowlist, policy revision, `unlimited|fact_only`, and upgrade/read-only behavior; TEST docs distinguish actual deployed SHA from uncommitted snapshots; unknown submissions remain non-retryable. No tests/gates are weakened and no optional systemd probe is connected to deploy-main.

### Wave1 final contract/UI/release closeout execution record (2026-09-06 19:13 UTC)

- Historical source timing (2026-09-06): the worker used branch `feat/v1-completion-wave1` at HEAD `e5a298905c474dfb8af7e6139ec74c68a98ec538` (base `5483218e`); this is not the current HEAD. Current source/release state must come from the parent snapshot and actual CI checkout; no historical record authorizes staging or deployment.
