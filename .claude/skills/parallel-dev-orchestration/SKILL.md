...
### 🤖 执行协议
Worker/Reviewer 执行协议详见 `parallel-dev-executor` skill。
```

**关键**：
- **指针优于正文**：口径指向唯一文档+节号，绝不在卡里复述设计内容（会与源漂移）
- **约束/红线/非目标前置且显著**：能不能被遵守，取决于够不够显眼
- **过程也要写**：分支基线、验证与关闭方式写进执行协议（指向 executor skill）
...
> 阶段 B 的「术」（5 步操作 + 粒度/依赖速查）回答的是「**怎么拆**」。本节回答「**为什么这样拆、怎样防跑偏**」—— 遇到拿不准的拆图判断，回这里查判据。
