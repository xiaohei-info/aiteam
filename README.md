...
**AI Team 是「云侧双控制面 + 用户本地数据面」的多 Agent 数字员工平台。** Operator 是多企业平台服务；Manager 按企业独立部署/绑定，一个 Manager 实例只服务一个企业；Agent 是每用户本机数据面。会话与执行全部在用户本机本地化，**内容不上传 Manager/Operator**。
...
| **企业端 Manager** | 每企业一套部署/绑定 | 当前企业成员账号与认证、专家/方案配置、成员级授权、企业共享 RAG、员工个人记忆、企业治理与计量汇总 | 当前企业 Manager DB + 企业数据空间 |
