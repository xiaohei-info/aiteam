...
AI Team 是一个基于 **Hermes Web UI** 二次开发的多 Agent 团队协作产品项目。

当前仓库的目标不是继续维护一个通用 Web UI，而是以现有 Hermes Web UI 代码为服务端/前端宿主基座，逐步落地：

- **Team Panel**：业务控制面
- **Agent Gateway**：运行时适配层
- 面向企业场景的多 Agent 私聊、群聊、协作、任务编排、治理与审计能力

## 当前项目结构

```text
/home/ubuntu/code/aiteam/
├── app/            # AI Team 主项目代码（由 Hermes Web UI 演化而来）
├── docs/           # 需求文档、业务方案、概要设计、详细设计、架构图
├── scripts/        # 项目级脚本预留目录
├── hermes-agent/   # 外部 Hermes Agent 源码仓库（独立 Git，不归 aiteam 主仓管理）
├── .gitignore
├── README.md
└── aiteam.code-workspace
```
