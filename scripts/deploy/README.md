# scripts/deploy —— PR merge 自动部署流水线

## 文件清单

| 文件 | 用途 | 何时改 |
|---|---|---|
| `scripts/deploy/run.sh` | 切分支 + pull --ff-only + `ctl.sh restart` + `/healthz` 冒烟 | **不动**：所有支流共用 |
| `.github/workflows/deploy-feature-v1.0.0.yml` | `feature/v1.0.0` PR merge → taiyi 的绑定关系 | **仅**创建新支流 / 改路由时复制并改三件套 |
| `scripts/deploy/README.md` | 本手册 | 新手指引 |
...
PR merged to feature/v1.0.0
  ...
  bash scripts/deploy/run.sh --branch feature/v1.0.0 --env test
        │
        ▼
