...
load_env() {
  ...
  validate_manager_production_env
...
validate_manager_production_env() {
  ...
  [[ -n "${AITEAM_MANAGER_TENANT_ID:-}" ]] || { echo "[ctl] ERROR: AITEAM_MANAGER_TENANT_ID is required for a production Manager deployment" >&2; exit 1; }
  [[ -n "${AITEAM_MANAGER_ENTERPRISE_ID:-}" ]] || { echo "[ctl] ERROR: AITEAM_MANAGER_ENTERPRISE_ID is required for a production Manager deployment" >&2; exit 1; }
}
  ...
  case "${service}" in
    ...
    manager)
      ...
      nohup setsid env \
        ...
        AITEAM_MANAGER_TENANT_ID="${AITEAM_MANAGER_TENANT_ID:-}" \
        AITEAM_MANAGER_ENTERPRISE_ID="${AITEAM_MANAGER_ENTERPRISE_ID:-}" \
    ...
    agent)
      ...
      nohup setsid env \
        ...
        -u AITEAM_MANAGER_TENANT_ID -u AITEAM_MANAGER_ENTERPRISE_ID -u AITEAM_HINDSIGHT_URL -u HINDSIGHT_URL -u HINDSIGHT_SERVICE_TOKEN -u HINDSIGHT_RECALL_PATH -u HINDSIGHT_RETAIN_PATH -u HINDSIGHT_DELETE_PATH -u HINDSIGHT_API_TOKEN -u HINDSIGHT_API_KEY -u HINDSIGHT_API_KEY_REF \
