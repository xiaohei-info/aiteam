...
export interface MemoryResult { success?: boolean; async?: boolean; memory_id?: string | null; employee_id?: string | null; content?: string | null; operation_id?: string | null; state?: "valid" | "invalidated" | null; items?: Array<Record<string, unknown>> | null; total?: number | null; [key: string]: unknown; }
/** Manager retain accepts the request and queues native Hindsight processing. */
export interface MemoryWriteAcknowledgement {
  success: true;
  async: true;
  operation_id: string;
}
export interface MemoryCreate { employee_id: string; content: string; metadata?: Record<string, unknown>; }
export interface MemoryUpdate { text?: string; content?: string; state?: "valid" | "invalidated"; }
