 ...
 * token 来源为本端 /api/auth/* login。401 触发 onUnauthorized → 清会话跳登录。
 */
...
const TOKEN_STORAGE_KEY = "aiteam.operation.token";
...
export function AppProviders({ children }: { children: ReactNode }): ReactNode {
  ...
  const i18n = useMemo(() => {
    ...
    // 运营端文案合并进共享 catalog（i18n.extend 已做 locale 合并）。
    instance.extend("zh-CN", operationMessages["zh-CN"]!);
    return instance;
  }, []);
