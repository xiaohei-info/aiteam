...
const operationRoot = resolve(import.meta.dirname, "../..");
const sourceRoot = join(operationRoot, "src");
...
describe("Operation zero-legacy UI gate", () => {
  ...
  it("does not retain the Tailwind build chain or shared legacy tokens", () => {
    ...
    const violations = files.flatMap((file) => {
      const source = readFileSync(join(operationRoot, file), "utf8");
