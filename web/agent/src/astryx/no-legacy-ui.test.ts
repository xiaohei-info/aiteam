...
const sourceRoot = join(agentRoot, "src");
...
describe("Agent zero-legacy UI gate", () => {
  ...
  it("does not retain the Tailwind build chain or shared legacy tokens", () => {
    ...
    const violations = files.flatMap((file) => {
      const source = readFileSync(join(agentRoot, file), "utf8");
