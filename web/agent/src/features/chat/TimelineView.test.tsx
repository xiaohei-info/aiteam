...
describe("TimelineView Pi cards", () => {
  ...
  it("renders todo statuses as a visual list", async () => {
    ...
    expect(card.querySelector('[data-timeline-status-icon="in_progress"]')).toHaveTextContent("⏳");
    expect(card.querySelector('[data-timeline-status-icon="waiting"]')).toHaveTextContent("⏸️");
    expect(card.querySelector('[data-timeline-status-icon="completed"]')).toHaveTextContent("✅");
  ...
  it("merges a tool result into its call card and shows the outcome without raw tool names", async () => {
    ...
    expect(toolCard?.querySelector('[data-timeline-status-icon="success"]')).toHaveTextContent("✅");
    expect(screen.queryByText("read")).not.toBeInTheDocument();
    expect(screen.getByRole("article", { name: "命令执行事件" }).querySelector('[data-timeline-status-icon="failure"]')).toHaveTextContent("❌");
