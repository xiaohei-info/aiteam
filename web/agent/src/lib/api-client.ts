...
export class AgentApiClient extends ApiClient {
  ...
  async login(req: AgentLoginRequest): Promise<AgentLoginResult> {
    const tenant_id = await this.resolveTenant(req.account, req.tenant_id, req.enterprise);
  ...
  private async resolveTenant(account: string, tenantId?: string | null, enterprise?: string | null): Promise<string> {
    if (tenantId) return tenantId;
    const result = await this.post<{ tenant_id: string }>("/api/auth/resolve-tenant-by-account", {
      body: { account, ...(enterprise?.trim() ? { enterprise: enterprise.trim() } : {}) },
    });
