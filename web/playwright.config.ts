...
export default defineConfig({
  ...
  webServer: external ? undefined : [
    ...
    {
      command:
        `cd .. && AITEAM_ENV=test MANAGER_PUBLIC_ORIGIN=${MANAGER_UI_ORIGIN} MANAGER_TENANT_ID=${MANAGER_TENANT_ID} DB_URL=${DB_URL} ADMIN_DB_URL=${ADMIN_DB_URL} SERVICE_TOKEN=${SERVICE_TOKEN} OPERATOR_URL=${OPERATOR_URL} ${E2E_PYTHON} server/run.py --tier manager --host 127.0.0.1 --port 8001`,
