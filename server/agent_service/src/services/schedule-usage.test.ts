...
test("usage aggregation is deterministic, bucketed, and contains no prompt data", () => {
  const summary = aggregateUsage({
    ...
    entries: [{ type: "message", message: { role: "assistant", usage: { input: 10, output: 5, cacheRead: 2, cacheWrite: 3, cost: { total: 999 }, responseModel: "upstream/other-model" }, content: [{ type: "text", text: "secret prompt" }] } } as never],
  ...
  assert.equal(summary.cost_total, "0.000010245000");
