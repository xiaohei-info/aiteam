...
function token(exp = Math.floor(Date.now() / 1000) + 60): { token: string; jwks: { keys: [{ kty: "RSA"; n: string; e: string; kid: string; alg: "RS256" }] } } {
  ...
  const payload = Buffer.from(JSON.stringify({ tenant_id: "tenant-a", user_id: "member-a", roles: ["member"], exp })).toString("base64url");
...
test("JWT authenticator verifies tenant identity and rejects expiry", async () => {
  const valid = token();
  const authenticate = createJwtAuthenticator({ jwks: valid.jwks });
  ...
  await assert.rejects(async () => createJwtAuthenticator({ jwks: expired.jwks })({ headers: { authorization: `Bearer ${expired.token}` } } as never), /expired/);
});
